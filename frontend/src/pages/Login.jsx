import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
import { useAuth } from '../App';

// Floating particle for background
function Particle({ delay, size }) {
  const x = Math.random() * 100;
  const y = Math.random() * 100;

  return (
    <motion.div
      className="absolute pointer-events-none text-green-500/20"
      style={{ left: `${x}%`, top: `${y}%`, fontSize: `${size}px` }}
      animate={{
        y: [0, -40, 0],
        x: [0, (Math.random() - 0.5) * 30, 0],
        rotate: [0, 360],
        opacity: [0.1, 0.4, 0.1],
      }}
      transition={{
        duration: 8 + Math.random() * 6,
        delay,
        repeat: Infinity,
        ease: 'easeInOut',
      }}
    >
      🍃
    </motion.div>
  );
}

export default function Login() {
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  const { login } = useAuth();

  const handleGoogleSignIn = async () => {
    setLoading(true);
    // Simulate auth delay then use demo login
    setTimeout(() => {
      login({
        uid: 'demo-user-001',
        displayName: 'Tanmay Sharma',
        email: 'tanmay@ecosense.dev',
        photoURL: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Tanmay',
      });
      setLoading(false);
      navigate('/dashboard');
    }, 1200);
  };

  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="min-h-screen flex items-center justify-center relative overflow-hidden px-4"
      role="main"
      id="main-content"
    >
      {/* Background */}
      <div className="absolute inset-0 bg-gradient-to-br from-eco-dark via-gray-900 to-eco-dark" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(34,197,94,0.06)_0%,_transparent_60%)]" />

      {/* Floating particles */}
      {[...Array(12)].map((_, i) => (
        <Particle key={i} delay={i * 0.8} size={12 + Math.random() * 10} />
      ))}

      {/* Login Card */}
      <motion.div
        initial={{ opacity: 0, y: 30, scale: 0.95 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ delay: 0.2, type: 'spring', damping: 20 }}
        className="relative z-10 w-full max-w-md"
      >
        <div className="bg-gray-900/90 backdrop-blur-xl border border-green-500/20 rounded-3xl p-8 shadow-[0_0_40px_rgba(34,197,94,0.08)]">
          {/* Logo & Branding */}
          <div className="text-center mb-8">
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              transition={{ delay: 0.3, type: 'spring' }}
              className="text-5xl mb-4"
            >
              🌿
            </motion.div>
            <h1 className="text-3xl font-black bg-gradient-to-r from-green-400 to-emerald-500 bg-clip-text text-transparent mb-2">
              EcoSense
            </h1>
            <p className="text-gray-500 text-sm">
              Sign in to track your carbon footprint
            </p>
          </div>

          {/* Divider */}
          <div className="flex items-center gap-3 mb-8">
            <div className="flex-1 h-px bg-gray-800" />
            <span className="text-xs text-gray-600 font-medium">CONTINUE WITH</span>
            <div className="flex-1 h-px bg-gray-800" />
          </div>

          {/* Google Sign In Button */}
          <button
            onClick={handleGoogleSignIn}
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-white/5 border border-gray-700 rounded-2xl text-white font-semibold hover:bg-white/10 hover:border-gray-600 transition-all active:scale-[0.98] disabled:opacity-50"
          >
            {loading ? (
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ repeat: Infinity, duration: 1, ease: 'linear' }}
                className="w-5 h-5 border-2 border-green-500 border-t-transparent rounded-full"
              />
            ) : (
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
            )}
            <span>{loading ? 'Signing in...' : 'Sign in with Google'}</span>
          </button>

          {/* Demo login option */}
          <button
            onClick={handleGoogleSignIn}
            className="w-full mt-3 px-6 py-3 text-sm font-medium text-green-400 border border-green-500/20 rounded-2xl hover:bg-green-500/5 transition-all"
          >
            🚀 Try Demo Account
          </button>

          {/* Privacy Note */}
          <p className="text-xs text-gray-600 text-center mt-8 leading-relaxed">
            By signing in, you agree to our{' '}
            <span className="text-gray-500 hover:text-green-400 cursor-pointer transition-colors">
              Terms of Service
            </span>{' '}
            and{' '}
            <span className="text-gray-500 hover:text-green-400 cursor-pointer transition-colors">
              Privacy Policy
            </span>
            . We only use your data to track your eco-impact.
          </p>
        </div>
      </motion.div>
    </motion.div>
  );
}
